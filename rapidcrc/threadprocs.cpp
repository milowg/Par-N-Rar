...
bool ThreadProc_Calc(bool *bSkipVerify, string sParfileName, vector<string> *m_vVerified, THREAD_PARAMS_CALC* pthread_params_calc, PROGRAM_STATUS *g_program_status, FILEINFO *g_fileinfo_list_first_item)
{
	...
	HANDLE hFile;
	BYTE buffer[MAX_BUFFER_SIZE_CALC];
	DWORD dwBytesRead;
	BOOL bSuccess;
	DWORD dwCrc32;
	...
	long lFileCount = 0;
	pFileinfo = g_fileinfo_list_first_item;
	while (pFileinfo->nextListItem != NULL)
	{
	...
	{
		std::vector<string>::iterator vIter = find(m_vVerified->begin(), m_vVerified->end(), pFileinfo->szFilename);
		...
		if ( vIter == m_vVerified->end() )
		{
			...
			if ( (pFileinfo->dwError == NO_ERROR) && (pFileinfo->qwFilesize != 0) && (bCalculateCrc || bCalculateMd5))
			{
				...
				{
					 if (*bSkipVerify == true) break; //Check for skipped verification

					pthread_params_calc->qwBytesReadCurFile  += dwBytesRead; //for progress bar
					pthread_params_calc->qwBytesReadAllFiles += dwBytesRead;
				...
				if (*bSkipVerify == true) return false; //Check for skipped verification
			...
			long lPercentage = (lFileCount / lNumFiles) * 100;
			strm.str(""); strm << lPercentage;
			CPnrMessage::SendParProgress(sParfileName, strm.str());

			if(bCalculateCrc)
				pFileinfo->dwCrc32Result = ~dwCrc32;

			if(bCalculateMd5)
				MD5_Final (pFileinfo->abMd5Result, &context);

			if (pFileinfo->dwError != 0)
			{
				bError = true;
				if ( vIter != m_vVerified->end() )
			...
			{
				if ( find(m_vVerified->begin(), m_vVerified->end(), pFileinfo->szFilename) == m_vVerified->end() )
					m_vVerified->push_back(pFileinfo->szFilename);
			}

			ShowResult(sParfileName, pFileinfo, g_program_status, false);
		}
		else
			ShowResult(sParfileName, pFileinfo, g_program_status, true);		

		pFileinfo = pFileinfo->nextListItem;
	}

	CPnrMessage::SendParProgress(sParfileName, "100");	
	return !bError;
}
...
BOOL ShowResult(string sParfileName, FILEINFO * pFileinfo, PROGRAM_STATUS *g_program_status, bool bAlreadyVerified)
{
	...
	string path, name;
	StringUtils::SplitFilename(pFileinfo->szFilename, path, name);

	strm << "Target: \"" << name << "\" - ";

	if (bAlreadyVerified)
		strm << "Already Verified";
	else
	{
		if (pFileinfo != NULL){
			if(pFileinfo->dwError != NO_ERROR){
				...
				{
					strm << "The found checksum for this file was not valid: Illegal CRC";
				}
				else if(pFileinfo->dwError == ERROR_FILE_NOT_FOUND)
				{
					strm << "The file could not be found: File not found";
				}
				else
				{
					strm << "Error occured with this file: " << CParNRarApp::ErrorMessage(pFileinfo->dwError);
				}
			}
			else{
