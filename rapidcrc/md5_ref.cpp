...
#include "stdafx.h"
#include "globals.h"	// includes correct md5_ref.h if USE_OSSL_REF is defined
...
static VOID MD5_Transform(DWORD [4], BYTE [64]);
static VOID Encode(BYTE *, DWORD *, UINT);
static VOID Decode(DWORD *, BYTE *, UINT);
static VOID MD5_memcpy(BYTE *, BYTE *, UINT);
static VOID MD5_memset(BYTE *, INT, UINT);
...
VOID MD5_Final (BYTE digest[16], MD5_CTX * context)
{
	...
	index = (UINT)((context->count[0] >> 3) & 0x3f); // get the number of bytes left in the buffer
	
	// is it possible to append the bit and the length to this buffer? otherwise we have to 'pad'
	// enough bytes to fill the next buffer so that there is only space left for the length
	padLen = (index < 56) ? (56 - index) : (120 - index);
	MD5_Update (context, PADDING, padLen);

	// Append length (before padding); ahhmmm.... we do this AFTER the padding, don't we?
...
// MD5 basic transformation. Transforms state based on block. Everytime 64 bytes are processed
...
static VOID Encode (BYTE * output, DWORD * input, UINT len)
{
	UINT i, j;
...
static VOID Decode (DWORD * output, BYTE * input, UINT len)
{
	UINT i, j;
...
static VOID MD5_memcpy (BYTE * output, BYTE * input, UINT len)
{
	/* UINT i;
...
static VOID MD5_memset (BYTE * output, INT value, UINT len)
{
	/*UINT i;
