...
public:
  ...
  bool Compute(string sParfileName);
...
template<class g>
inline bool ReedSolomon<g>::Compute(string sParfileName)
{
	...
	{
		strm.str(""); strm << "Not enough recovery blocks." ;
		CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		return false;
	}
	else if (outcount == 0)
	{
		strm.str(""); strm << "No output blocks." ;
		CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		return false;
	}

	strm.str(""); strm << "Computing Reed Solomon matrix." ;
	CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
	CPnrMessage::SendParStatus(m_sParfileName, "Constructing Matrix");
	...
	*/
	...
	for (unsigned int row=0; row<datamissing; row++)
	{
		...
		//strm.str(""); strm << "Constructing: " << progress/10 << '.' << progress%10 << "%" << flush;
		//(new CMessageString(strm.str().c_str()))->PostAsWParam(g_hwndMain, WM_PARNRAR_PROGRESS, progress);
		strm.str(""); strm << progress/10 << '.' << progress%10;
	...
	strm.str(""); strm << "Constructing: done." ;
	CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
	...
	if (datamissing > 0)
	{
		...
		strm.str(""); strm << "GaussElim done" ;  
		CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		delete [] rightmatrix;
		strm.str(""); strm << "Exiting RS::Compute" ;    
		CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		return success;
	}
...
inline bool ReedSolomon<g>::GaussElim(unsigned int rows, unsigned int leftcols, G *leftmatrix, G *rightmatrix, unsigned int datamissing)
{
	...
	ostringstream strm;
	CPnrMessage::SendParStatus(m_sParfileName, "Solving Matrix");
	...
	for (unsigned int row=0; row<datamissing; row++)
	{
		...
		if (pivotvalue == 0)
		{
			strm.str(""); strm << "RS computation error." ;
			CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
			return false;
		}
		...
		for (unsigned int row2=0; row2<rows; row2++)
		{
			...
			if (progress != newprogress)
			{
				...
				//strm.str(""); strm << "Solving: " << progress/10 << '.' << progress%10 << "%" << flush;
				//(new CMessageString(strm.str().c_str()))->PostAsWParam(g_hwndMain, WM_PARNRAR_PROGRESS, progress);
				strm.str(""); strm << progress/10 << '.' << progress%10;
				CPnrMessage::SendParProgress(m_sParfileName, strm.str().c_str());
			}
