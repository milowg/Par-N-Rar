...
Par1Repairer::Par1Repairer(string sParfileName) : IParRepairer(sParfileName)
{
	filelist = 0;
	filelistsize = 0;
...
set<string> Par1Repairer::GetContainedFiles(string filename)
{
	...
	if (!LoadRecoveryFile(filename, false))
	...
	while (sf != sourcefiles.end())
	{
		...
		setContainedFiles.insert(path + sourcefile->FileName());
...
Result Par1Repairer::Process(const string sParfileName, const string par1filename, bool dorepair, string &sParDone)
{
	...
	if (!LoadOtherRecoveryFiles(par1filename, lNumOtherFiles))
		if (m_bSkipVerify) 
	...
	if (!LoadRecoveryFile(searchpath + name, true))
		return eLogicError;
	...
	if (completefilecount<sourcefiles.size())
	{
		...
		if (dorepair)
		{
			...
			if (completefilecount<sourcefiles.size())
			{
				...
				strm.str(""); strm << "Verifying repaired files:";
				CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
...
size_t Par1Repairer::MemoryLimit()
{
...
bool Par1Repairer::LoadRecoveryFile(string filename, bool bSendMessages)
{
	ostringstream strm;
	...
	if (bSendMessages)
	{
		if (havevolume)
		{
			strm.str(""); strm << "Loaded recovery volume " << volumenumber ;
			CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		}
		else
		{
			strm.str(""); strm << "No new recovery volumes found" ;
			CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		}
	}
...
vector<string> Par1Repairer::GetOtherFiles(string filename)
{
...
bool Par1Repairer::LoadOtherRecoveryFiles(string filename, long &lNumOtherFiles)
{
	// Split the original PAR filename into path and name parts
	string path;
	string name;
	...
	for (list<string>::const_iterator s=goodFiles.begin(); s!=goodFiles.end(); ++s)
	{
		...
		CPnrMessage::SendParStatus(m_sParfileName, strm.str().c_str());
		LoadRecoveryFile(filename, true);		
		strm.str(""); strm << CALC_PROGRESS(iIter, goodFiles.size() + 1);
		...
		if (sourceiterator != sourcefiles.end())
		{
			...
			if (filesize > 16384)
			{
				...
				while (offset < filesize)
				{
					...
					{
						//strm.str(""); strm << "Scanning: \"" << name << "\": " << newfraction/10 << '.' << newfraction%10 << "%" << flush;
						//(new CMessageString(strm.str().c_str()))->PostAsWParam(g_hwndMain, WM_PARNRAR_PROGRESS, newfraction);
...
void Par1Repairer::UpdateVerificationResults(void)
{
	...
	while (sf != sourcefiles.end())
	{
		...
		if (sourcefile->GetCompleteFile() != 0)
		{
			...
			if ( find(m_vVerified.begin(), m_vVerified.end(), sourcefile->FileName()) == m_vVerified.end() )
				m_vVerified.push_back(sourcefile->FileName());
		}
		else
		{
			// Does the target file exist
...
bool Par1Repairer::CheckVerificationResults(string &sParDone)
{
	ostringstream strm;
		...
		missingfilecount > 0)
	{
		...
		if (recoveryblocks.size() >= damagedfilecount+missingfilecount)
		{
			...
			{
				strm.str(""); strm << damagedfilecount+missingfilecount
					<< " recovery files will be used to repair." ;
				CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
			}
			else if (recoveryblocks.size())
			{
				strm.str(""); strm << "None of the recovery files will be used for the repair." ;
				CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
			}
		...
		{			
			strm.str(""); strm << "Missing " << damagedfilecount+missingfilecount - recoveryblocks.size() << " Recovery File(s)";			
			sParDone = strm.str();
			strm.str(""); strm << "Repair is not possible." << endl << "You need " << damagedfilecount+missingfilecount - recoveryblocks.size() << " more recovery files to be able to repair.";
			CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
			return false;
		}
	}
	...
	if (verifylist.size() > 0)
	{
		...
		while (inputblock != inputblocks.end())       
		{
			...
			for (u32 outputindex=0; outputindex<verifylist.size(); outputindex++)
			{
				...
				if (oldfraction != newfraction)
				{
					//strm.str(""); strm << "Repairing: " << newfraction/10 << '.' << newfraction%10 << "%" << flush;
					//(new CMessageString(strm.str().c_str()))->PostAsWParam(g_hwndMain, WM_PARNRAR_PROGRESS, newfraction);
					strm.str(""); strm << newfraction/10 << '.' << newfraction%10;
					CPnrMessage::SendParProgress(m_sParfileName, strm.str().c_str());
				}
...
bool Par1Repairer::VerifyTargetFiles(void)
{
	...
	long lIter = 1;
	for (list<Par1RepairerSourceFile*>::iterator sf = verifylist.begin(); sf != verifylist.end(); ++sf)
	{
		strm.str(""); strm << "Verifying repaired files (" << lIter++ << "/" << verifylist.size() << ")";
		CPnrMessage::SendParStatus(m_sParfileName, strm.str().c_str());
...
bool Par1Repairer::Repair(string sDir, string sFile, vector<string> &vFiles, vector<string> &vRenamedFiles, vector<string> &vVerifiedFiles, ParSettings &fileSettings, vector<string> &vDoneFiles, bool bVerifyOnly)
{
	bool bRet = false;
	ostringstream strm;
	string sParDone = "";

	strm.str(""); strm << "Checking PAR1 file " << sDir << sFile ;
	CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);

	Result  result = Process(m_sParfileName, sDir + sFile, !bVerifyOnly, sParDone);
	if (result == eSuccess)
	{		
		bRet = true;
	...
	{
		strm.str(""); strm << "Error verifying PAR file: " << CommandLine::GetEnglishResult(result);
		CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		if (sParDone == "")
			sParDone = "Error";
	}
	...
	vector<Par1RepairerSourceFile*>::iterator iterVerified = sourcefiles.begin();
	while (iterVerified != sourcefiles.end())
	{
		Par1RepairerSourceFile *p1sf = (*iterVerified);
		vVerifiedFiles.push_back( p1sf->FileName() );
		++iterVerified;
	}

	//If there are no files to extract, return false and show error
	if (sourcefiles.size() == 0)
	{
		strm.str(""); strm << "No files to extract";
		CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		result = eRepairNotPossible;
		sParDone = "Error";
		bRet = false;
	}
