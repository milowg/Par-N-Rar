...
Par2Repairer::Par2Repairer(string sParfileName) : IParRepairer(sParfileName), m_bVerificationDone(false)
{
	firstpacket = true;
	mainpacket = 0;
	creatorpacket = 0;
...
set<string> Par2Repairer::GetContainedFiles(string filename)
{
	string path;
	string name;
	DiskFile::SplitFilename(filename, path, name);
	set<string> setContainedFiles;

	if (!LoadPacketsFromFile(filename, false, 0, 0))
		return setContainedFiles;
	if (m_bSkipVerify) return setContainedFiles; //Check for skipped verification

	//Make sure file exists
	if (_access(filename.c_str(), 0) == -1)
		return setContainedFiles;

	//Make sure we have valid packets
	if (mainpacket == NULL)
		return setContainedFiles;
	...
	for (u32 filenumber=0; filenumber<mainpacket->TotalFileCount(); filenumber++)
	{
		...
		if (sourcefile)
		{
			if (sourcefile->ComputeTargetFileName(searchpath) == false)
				return setContainedFiles;

			setContainedFiles.insert(path + sourcefile->TargetFileName());
		}
	}
...
// Get other PAR2 files with names based on the original PAR2 file
vector<string> Par2Repairer::GetOtherFiles(string filename)
{
	vector<string> vFileMap;
	...
	//If file is .volXXX.PAR2, .par2, then only use this one
	//		 (compare files in the set and see if they are the same)
	set<string> setContainedFiles;
	if (name.empty())
		setContainedFiles = GetContainedFiles(filename);

	string wildcard = name + ".*.par2";
	...
	{				
		if (name.empty())
		{
			//Check whether this file contains the same files
			Par2Repairer *repairTemp = new Par2Repairer("");
			set<string> setCheckContainedFiles = repairTemp->GetContainedFiles(*s);
			delete repairTemp;

			//If these sets are not the same, do not add to filemap list
			if (setCheckContainedFiles != setContainedFiles)
				continue;
		}
		vFileMap.push_back(*s);
	}
	delete files;
...
Result Par2Repairer::Process(const string sParfileName, const string par2filename, bool dorepair, ParSettings &fileSettings, string &sParDone)
{
	...
	long lNumOtherFiles = 0;
	if (!LoadPacketsFromOtherFiles(par2filename, lNumOtherFiles))
		if (m_bSkipVerify) 
			return eSkipped; //Check for skipped verification
		else
			return eLogicError;

	// Load packets from the main PAR2 file
	strm.str(""); strm << "Loading PARs (" << lNumOtherFiles + 1 << "/" << lNumOtherFiles + 1 << ")";
	CPnrMessage::SendParStatus(m_sParfileName, strm.str().c_str());
	if (!LoadPacketsFromFile(searchpath + name, true, CALC_PROGRESS(lNumOtherFiles, lNumOtherFiles+1), lNumOtherFiles + 1))
		return eLogicError;
	if (m_bSkipVerify) return eSkipped; //Check for skipped verification
	...
	if (!sourcefiles.size ())
	{
		...
		strm.str(""); strm << "Verifying source files:";
	...
	m_bVerificationDone = true;
	if (!VerifySourceFiles(fileSettings))
		if (m_bSkipVerify) 
			return eSkipped; //Check for skipped verification
		else
			return eFileIOError;
	...
	{
		strm.str(""); strm << "Scanning extra files:";
		CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
	}

	// Find out how much data we have found
	UpdateVerificationResults(fileSettings);

	if (m_bSkipVerify) return eSkipped; //Check for skipped verification

	// Check the verification results and report the results
	if (!CheckVerificationResults(sParDone))
	...
	if (completefilecount<mainpacket->RecoverableFileCount())
	{
		...
		if (dorepair)
		{
			...
			if (completefilecount<mainpacket->RecoverableFileCount())
			{
				...
				if (!VerifyTargetFiles(fileSettings))
				{
					// Delete all of the partly reconstructed files
					DeleteIncompleteTargetFiles();
					return eFileIOError;
			...
			if (completefilecount<mainpacket->RecoverableFileCount())
			{
				strm.str(""); strm << "Repair Failed" ;
				CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
				return eRepairFailed;
			}
			else
			{
				strm.str(""); strm << "Repair complete." ;
				CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
				CPnrMessage::SendParProgress(m_sParfileName, "100");
			}
		}
...
size_t Par2Repairer::MemoryLimit()
{
	...
	return (size_t)(TotalPhysicalMemory / 2);

}

// Load the packets from the specified file
bool Par2Repairer::LoadPacketsFromFile(string filename, bool bSendMessage, long lProgress, long lProgressPercent)
{
	ostringstream strm;
	...
	DiskFile *diskfile = new DiskFile(m_sParfileName);
...

	{
		...
		if (bSendMessage)
		{
			strm.str(""); strm << "Loading PARs \"" << name << "\"." ;
			CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		}
	}
	...
	if (filesize > 0)
	{
		...
		while (offset + sizeof(PACKET_HEADER) <= filesize)
		{
			...
			if (oldfraction != newfraction)
			{
				if (bSendMessage)
				{
					newfraction = (u32)(newfraction / (u32)lProgressPercent + (u32)lProgress);
					strm.str(""); strm << newfraction/10 << '.' << newfraction%10;
					CPnrMessage::SendParProgress(m_sParfileName, strm.str().c_str());
				}
				progress = offset;
			}
			...
			if (m_bSkipVerify) break; //Check for skipped verification
	...
	if (m_bSkipVerify) return false; //Check for skipped verification
	...
	if (packets > 0)
	{
		if (bSendMessage)
		{
			strm.str(""); strm << "Loaded " << packets << " new packets";
			CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
			if (recoverypackets > 0)
			{
				strm.str(""); strm << " including " << recoverypackets << " recovery blocks";
				CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
			}
		}
	...
	else
	{
		if (bSendMessage)
		{
			strm.str(""); strm << "No new packets found";
			CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		}
		delete diskfile;
	}
...
// Load packets from other PAR2 files with names based on the original PAR2 file
bool Par2Repairer::LoadPacketsFromOtherFiles(string filename, long &lNumOtherFiles)
{
	// Split the original PAR2 filename into path and name parts
	string path;
	string name;
	...
	int iIter = 1;
	lNumOtherFiles = goodFiles.size();
	for (list<string>::const_iterator s=goodFiles.begin(); s!=goodFiles.end(); ++s)
	{
		strm.str(""); strm << "Loading PARs (" << iIter << "/" << goodFiles.size() + 1 << ")";
		CPnrMessage::SendParStatus(m_sParfileName, strm.str().c_str());

		if (m_bSkipVerify) return false; //Check for skipped verification

		LoadPacketsFromFile(*s, true, CALC_PROGRESS(iIter-1, goodFiles.size() + 1), goodFiles.size() + 1);
...
bool Par2Repairer::LoadPacketsFromExtraFiles(const list<CommandLine::ExtraFile> &extrafiles)
{
	...
	for (ExtraFileIterator i=extrafiles.begin(); i!=extrafiles.end(); i++)
	{
		...
		{
			if (m_bSkipVerify) return false; //Check for skipped verification
			LoadPacketsFromFile(filename, true, (u32)(iIter / extrafiles.size()), extrafiles.size());
		}
		iIter++;
	}
...
bool Par2Repairer::CheckPacketConsistency(void)
{
	...
	if (0 == mainpacket)
	{
		...
		strm.str(""); strm << "Main packet not found.";
		CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		return false;
	}
	...
	// and discard any that don't
	{
		...
		while (rp != recoverypacketmap.end())
		{
			...
			{
				strm.str(""); strm << "Incorrect sized recovery block for exponent " << rp->second->Exponent() << " discarded";
				CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
	...
	// verification packet has the wrong number of entries and discard them.
	{
		...
		while (sf != sourcefilemap.end())
		{
			...
			if ((filesize + blocksize-1) / blocksize != (u64)blockcount)
			{
				...
				strm.str(""); strm << "Incorrectly sized verification packet for \"" << descriptionpacket->FileName() << "\" discarded" ;
				CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
	...
	strm.str(""); strm << "The block size used was "
		<< blocksize
		<< " bytes."
		;
	CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
...
bool Par2Repairer::CreateSourceFileList(void)
{
	...
	for (u32 filenumber=0; filenumber<mainpacket->TotalFileCount(); filenumber++)
	{
		...
		//If the file is valid, then add it to sourcefiles. Otherwise, lower the valid files count by one
		if (sourcefile && sourcefile->GetVerificationPacket() != NULL)
...
bool Par2Repairer::AllocateSourceBlocks(void)
{
	...
	if (sourceblockcount > 0)
	{
		...
		strm.str(""); strm << "There are a total of "
			<< sourceblockcount
			<< " data blocks."
			;
		CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);

		strm.str(""); strm << "The total size of the data files is "
			<< totalsize
			<< " bytes."
			;
		CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
...
// Attempt to verify all of the source files
bool Par2Repairer::VerifySourceFiles(ParSettings &fileSettings)
{
	...
	while (sf != sourcefiles.end())
	{
		...
		else
		{
			...
			{
				strm.str(""); strm << "No details available for recoverable file number " << filenumber+1 << ". Recovery will not be possible." ;
				CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
			...
			{
				strm.str(""); strm << "No details available for non-recoverable file number " << filenumber - mainpacket->RecoverableFileCount() + 1 ;
				CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
			}
		}
	...
	{
		lIter++;
		strm.str(""); strm << "Verifying (" << lIter << "/" << sourcefiles.size() << ")";
		CPnrMessage::SendParStatus(m_sParfileName, strm.str().c_str());
		...
		string path, name;
		DiskFile::SplitFilename(sourcefile->TargetFileName(), path, name);

		DiskFile *diskfile = new DiskFile(m_sParfileName);
		std::vector<string>::iterator vIter = find(m_vVerified.begin(), m_vVerified.end(), filename);

		// Does the target file exist
		if (diskfile->Open(filename))
		{
			//Make sure file is not already verified
			if ( sourcefile->GetTargetExists () )
			{
				strm.str(""); strm << "Target: \"" << name << "\" - already verified." ;
				if ( vIter == m_vVerified.end() )
					strm << " File is not complete.";
				CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
			}
			else
			{
				...
				if (diskFileMap.Find (diskfile->FileName ()))
				{
				...
				if (!VerifyDataFile(diskfile, sourcefile, CALC_PROGRESS(lIter, sourcefiles.size()), sourcefiles.size()))
					finalresult = false;
			}
			// We have finished with the file for now
			diskfile->Close();
		...
		else
		{
			...
			string sSplitSearch = filename + ".0*";

			HANDLE hFind = FindFirstFile(sSplitSearch.c_str(), &FindFileData);
			bool bAddedSplitFile = false;
			while (hFind != INVALID_HANDLE_VALUE) 
			{	
				//It is a split file!
				m_bIsSplit = true;
				
				//Make sure we haven't already added this file
				if ( find(m_vSplitFilesAdded.begin(), m_vSplitFilesAdded.end(), path + FindFileData.cFileName) == m_vSplitFilesAdded.end() )				
				{
					DescriptionPacket *packet = new DescriptionPacket;

					//Add split file to sourcefiles
					Par2RepairerSourceFile *sfSplit = new Par2RepairerSourceFile(sourcefile->GetDescriptionPacket(), NULL);
					DiskFile::SplitFilename(sSplitSearch, path, name);
					sfSplit->SetTargetFileName(path + FindFileData.cFileName);
					sourcefiles.push_back(sfSplit);
					m_vSplitFilesAdded.push_back(path + FindFileData.cFileName);
					bAddedSplitFile = true;
				}
				if (!FindNextFile(hFind, &FindFileData))
					break;
			}
			FindClose(hFind);		
			if (m_bIsSplit && bAddedSplitFile)
			{
				//We just added at least one split file to our sourcefiles. Call this function again
				return VerifySourceFiles(fileSettings);
			}

			//Remove from verified list			
			if ( vIter != m_vVerified.end() )
				m_vVerified.erase(vIter);

			strm.str(""); strm << "Target: \"" << name << "\" - missing." ;
			CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		}
...
bool Par2Repairer::VerifyExtraFiles(const list<CommandLine::ExtraFile> &extrafiles, ParSettings &fileSettings)
{
	ostringstream strm;
	long lIter = 0;
	for (ExtraFileIterator i=extrafiles.begin(); i!=extrafiles.end() && completefilecount<mainpacket->RecoverableFileCount(); ++i)
	{
		lIter++;
		string filename = i->FileName();
			...
			string::npos == filename.find(".PAR2"))
		{
			...
			if (diskFileMap.Find(filename) == 0)
			{
				...
				// Do the actual verification
				VerifyDataFile(diskfile, 0, CALC_PROGRESS(lIter, extrafiles.size()), extrafiles.size());
				// Ignore errors
...
bool Par2Repairer::VerifyDataFile(DiskFile *diskfile, Par2RepairerSourceFile *sourcefile, long lProgress, long lProgressPercent)
{
	...
	if (blockverifiable)
	{
		...
		if (!ScanDataFile(diskfile,   // [in]      The file to scan
			...
			hashfull,   // [out]
			hash16k,    // [out]
			count, lProgress, lProgressPercent))     // [out]
			return false;
	...
	if (unverifiablesourcefiles.size() > 0)
	{
		...
		while (sf != unverifiablesourcefiles.end())
		{
			...
			{
				strm.str(""); strm << diskfile->FileName() << " is a perfect match for " << sourcefile->GetDescriptionPacket()->FileName() ;
				CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
...
bool Par2Repairer::ScanDataFile(DiskFile                *diskfile,    // [in]
								...
								MD5Hash                 &hash16k,     // [out]
								u32                     &count, long lProgress, long lProgressPercent)       // [out]
{
	...
	if (count > 0)
	{
		...
		else
		{
			...
			if (originalsourcefile == sourcefile)
			{
				strm.str(""); strm << "Target: \"" << name << "\" - found." ;
				CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
			}
			// Were we scanning the target file or an extra file
			else if (originalsourcefile != 0)
...
// Find out how much data we have found
void Par2Repairer::UpdateVerificationResults(ParSettings &fileSettings)
{
	availableblockcount = 0;
	missingblockcount = 0;
	...
	while (sf != sourcefiles.end() && filenumber < mainpacket->TotalFileCount())
	{
		...
		if (sourcefile->GetCompleteFile() != 0)
		{
			...
			if ( find(m_vVerified.begin(), m_vVerified.end(), sourcefile->TargetFileName()) == m_vVerified.end() )
				m_vVerified.push_back(sourcefile->TargetFileName());
...
// Check the verification results and report the results 
bool Par2Repairer::CheckVerificationResults(string &sParDone)
{
	ostringstream strm;
	...
	if (completefilecount < mainpacket->RecoverableFileCount() ||
		...
		missingfilecount > 0)
	{
		strm.str(""); strm << "Repair is required." ;
		CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		if (renamedfilecount > 0)
		{
			strm.str(""); strm << renamedfilecount << " file(s) have the wrong name." ;
			CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		}
		if (missingfilecount > 0)
		{
			strm.str(""); strm << missingfilecount << " file(s) are missing." ;
			CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		}
		if (damagedfilecount > 0)
		{
			strm.str(""); strm << damagedfilecount << " file(s) exist but are damaged." ;
			CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		}
		if (completefilecount > 0)
		{
			strm.str(""); strm << completefilecount << " file(s) are ok." ;
			CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		}

		strm.str(""); strm << "You have " << availableblockcount 
			<< " out of " << sourceblockcount 
			<< " data blocks available." ;
		CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		if (recoverypacketmap.size() > 0)
		{
			strm.str(""); strm << "You have " << (u32)recoverypacketmap.size() 
		...
		{
			strm.str(""); strm << "Repair is possible." ;
			CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);

			if (recoverypacketmap.size() > missingblockcount)
			{
				strm.str(""); strm << "You have an excess of " 
			...
			{
				strm.str(""); strm << missingblockcount
					<< " recovery blocks will be used to repair." ;
				CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
			}
			else if (recoverypacketmap.size())
			{
				strm.str(""); strm << "None of the recovery blocks will be used for the repair." ;
				CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
			}
		...
		{
			strm.str(""); strm << "Missing " << missingblockcount - recoverypacketmap.size() << " Recovery Block(s)";
			sParDone = strm.str();			

			strm.str(""); strm << "Repair is not possible. You need " << missingblockcount - recoverypacketmap.size() << " more recovery blocks to be able to repair." ;
			CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
			return false;
		}
	}
	...
	{
		strm.str(""); strm << "All files are correct, repair is not required." ;
		CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
...
bool Par2Repairer::RenameTargetFiles(void)
{
	...
	while (sf != sourcefiles.end() && filenumber < mainpacket->TotalFileCount())
	{
			...
			sourcefile->GetTargetFile() != sourcefile->GetCompleteFile())
		{
			...
			DiskFile *targetfileRenamed = new DiskFile(*targetfile);
			bool success = diskFileMapRenamed.Insert(targetfileRenamed);
			assert(success);
			success = diskFileMap.Insert(targetfile);
			assert(success);
...
bool Par2Repairer::CreateTargetFiles(void)
{
	...
	while (sf != sourcefiles.end() && filenumber < mainpacket->TotalFileCount())
	{
		...
		if (!sourcefile->GetTargetExists())
		{
			DiskFile *targetfile = new DiskFile(m_sParfileName);
			string filename = sourcefile->TargetFileName();
			u64 filesize = sourcefile->GetDescriptionPacket()->FileSize();
...
bool Par2Repairer::ComputeRSmatrix(void)
{
	...
	bool success = rs.Compute(m_sParfileName);
...
bool Par2Repairer::AllocateBuffers(size_t memorylimit)
{
	...
	{
		strm.str(""); strm << "Could not allocate buffer memory." ;
		CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		return false;
	}
...
bool Par2Repairer::ProcessData(u64 blockoffset, size_t blocklength)
{
	...
	if (missingblockcount > 0)
	{
		...
		{
			if (m_bIsSplit)
				CPnrMessage::SendParStatus(m_sParfileName, "Joining");
			else
				CPnrMessage::SendParStatus(m_sParfileName, "Repairing");
	...
	else
	{
		...
		CPnrMessage::SendParStatus(m_sParfileName, "Processing");
	...
	strm.str(""); strm << "Writing recovered data";
	CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
	...
	strm.str(""); strm << "Wrote " << totalwritten << " bytes to disk" ;
	CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
...
// Verify that all of the reconstructed target files are now correct
bool Par2Repairer::VerifyTargetFiles(ParSettings &fileSettings)
{
	bool finalresult = true;
	ostringstream strm;
	...
	string sType = "repaired";
	if (m_bIsSplit)
		sType = "joined";
	
	strm.str(""); strm << "Verifying " << sType << " files:";
	CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);

	// Iterate through each file in the verification list
	long lIter = 0;
	for (vector<Par2RepairerSourceFile*>::iterator sf = verifylist.begin(); sf != verifylist.end(); ++sf)
	{
		lIter++;
		strm.str(""); strm << "Verifying " << sType << " files (" << lIter << "/" << verifylist.size() << ")";
		CPnrMessage::SendParStatus(m_sParfileName, strm.str().c_str());
		...
		if (!VerifyDataFile(targetfile, sourcefile, CALC_PROGRESS(lIter, verifylist.size()), verifylist.size()))
			finalresult = false;
...
bool Par2Repairer::Repair(string sDir, string sFile, vector<string> &vFiles, vector<string> &vRenamedFiles, vector<string> &vVerifiedFiles, ParSettings &fileSettings, vector<string> &vDoneFiles, bool bVerifyOnly)
{
	bool bRet = false;
	ostringstream strm;
	string sParDone = "";

	strm.str(""); strm << "Checking PAR2 file " << sDir << sFile ;
	CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);

	Result  result = Process(sFile, sDir + sFile, !bVerifyOnly, fileSettings, sParDone);
	if (result == eSuccess)
	{
		bRet = true;
	}
	else if (result == eRepairNotPossible)
	{
		//Already sent message
	}
	else if (result == eRepairPossible)
	{
		sParDone = "Repair Possible";
	}
	else if (result == eSkipped)
	{	
	...
	{
		strm.str(""); strm << "Error verifying PAR2 file: " << CommandLine::GetEnglishResult(result);
		CPnrMessage::SendParAddDetails(m_sParfileName, strm.str(), true);
		if (sParDone == "")
			sParDone = "Error";
	}

	//Get a list of all files looked at
	map<string, DiskFile*>::iterator fi = diskFileMap.diskfilemap.begin();
	while (fi != diskFileMap.diskfilemap.end())
	{
		vFiles.push_back( (*fi).first.c_str() );
		++fi;
	}

	//Include renamed original files (filename.ext.1)
	fi = diskFileMapRenamed.diskfilemap.begin();
	while (fi != diskFileMapRenamed.diskfilemap.end())
	{
		vFiles.push_back( (*fi).first.c_str() );
		vRenamedFiles.push_back( (*fi).first.c_str() );
		++fi;
	}

	//Get a list of all non-PAR files looked at
	vector<Par2RepairerSourceFile*>::iterator iterVerified = sourcefiles.begin();
	while (iterVerified != sourcefiles.end())
	{
		Par2RepairerSourceFile *p2sf = (*iterVerified);

		//If this is a split file, we want to return the done file (ie the put together file)
		if (m_bIsSplit)
		{
			//If this is a partial (split) file, do not add it
			if (find(m_vSplitFilesAdded.begin(), m_vSplitFilesAdded.end(), p2sf->TargetFileName()) == m_vSplitFilesAdded.end())
			{
				vDoneFiles.push_back( p2sf->TargetFileName() );
			}
		}
		
		vVerifiedFiles.push_back( p2sf->TargetFileName() );
		++iterVerified;
	}

	if (sParDone == "" && result == eRepairNotPossible)
		sParDone = "Repair not possible";
	if (sParDone != "")
	{
		eStatus status = DONE;
		if (bVerifyOnly)
			status = (result == eSuccess ? DONE : VERIFIED);

		CPnrMessage::SendParDone(m_sParfileName, sParDone, false, status);
	}
	return bRet;
}
