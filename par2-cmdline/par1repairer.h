...
class Par1Repairer : IParRepairer
{
public:
	Par1Repairer(string sParfileName);
	~Par1Repairer(void);

	vector<string> GetOtherFiles(string filename);
	Result Process(const string sParfileName, const string par1filename, bool dorepair, string &sParDone);
	vector<Par1RepairerSourceFile*> sourcefiles;
	DiskFileMap diskfilemap, diskfilemapRenamed;             // Map from filename to DiskFile  
...
protected:
	...
	bool LoadRecoveryFile(string filename, bool bSendMessages);

	// Load other PAR files related to the main PAR file
	bool LoadOtherRecoveryFiles(string filename, long &lNumOtherFiles);
