...
class Par2Repairer : IParRepairer
{
public:
	Par2Repairer(string sParfileName);
	~Par2Repairer(void);

	Result Process(const string sParfileName, const string par2filename, bool dorepair, ParSettings &fileSettings, string &sParDone);
	bool Repair(string sDir, string sFile, vector<string> &vFiles, vector<string> &vRenamedFiles, vector<string> &vVerifiedFiles, ParSettings &fileSettings, vector<string> &vDoneFiles, bool bVerifyOnly);

	size_t MemoryLimit();

	vector<Par2RepairerSourceFile*> sourcefiles;  // The source files
	DiskFileMap diskFileMap, diskFileMapRenamed;
	vector<string> GetOtherFiles(string filename);	
	set<string> GetContainedFiles(string filename);
protected:
	vector<string> m_vVerified;

	//For split files
	bool m_bIsSplit;
	vector<string> m_vSplitFilesAdded;

	// Steps in verifying and repairing files:	

	// Load packets from the specified file
	bool LoadPacketsFromFile(string filename, bool bSendMessage, long lProgress, long lProgressPercent);
	...
	bool LoadPacketsFromOtherFiles(string filename, long &lNumOtherFiles);
	...
	bool VerifySourceFiles(ParSettings &fileSettings);

	// Scan any extra files specified on the command line
	bool VerifyExtraFiles(const list<CommandLine::ExtraFile> &extrafiles, ParSettings &fileSettings);

	// Attempt to match the data in the DiskFile with the source file
	bool VerifyDataFile(DiskFile *diskfile, Par2RepairerSourceFile *sourcefile, long lProgress, long lProgressPercent);
	...
	bool ScanDataFile(DiskFile                *diskfile,   // [in]     The file being scanned
		...
		long lProgress, long lProgressPercent);

	// Find out how much data we have found
	void UpdateVerificationResults(ParSettings &fileSettings);

	// Check the verification results and report the results 
	bool CheckVerificationResults(string &sParDone);
	...
	bool VerifyTargetFiles(ParSettings &fileSettings);
