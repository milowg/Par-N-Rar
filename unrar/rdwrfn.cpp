...
void ComprDataIO::ShowUnpRead(Int64 ArcPos,Int64 ArcSize)
{
  if (ShowProgress && SrcFile!=NULL)
  {
    ...
    if (!SrcArc->Volume)
    {
      ...
      {
		ostringstream strm;
        //strm.str(""); strm << "Extracting: " << CurPercent << "%";
		//(new CMessageString(strm.str().c_str()))->PostAsWParam(g_hwndMain, WM_PARNRAR_PROGRESS, CurPercent * 10);
		strm.str(""); strm << CurPercent;
		CPnrMessage::SendParProgress(SrcArc->m_sParfileName, strm.str().c_str());
