...
void CreatePath(string sParfileName, const char *Path,const wchar *PathW,bool SkipLastName)
{
#ifdef _WIN_32
	uint DirAttr=0;
#else
	...
	for (int PosW=0;;PosW++)
	{
		...
		if (Wide && PathW[PosW]==CPATHDIVIDER || !Wide && *s==CPATHDIVIDER)
		{
			...
			{
#ifndef GUI
				mprintf(sParfileName, St(MCreatDir),DirName);
				mprintf(sParfileName, " %s",St(MOk));
#endif
			}
		}
		if (!IgnoreAscii)
...
uint CalcFileCRC(File *SrcFile,Int64 Size)
{
	...
	Array<byte> Data(SrcFile->m_sParfileName, BufSize);
	Int64 BlockCount=0;
