...
void ExtractStreams(Archive &Arc,char *FileName,wchar *FileNameW)
{
	...
	File CurFile(Arc.m_pvRarFiles, Arc.m_sParfileName);
	if (CurFile.WCreate(StreamName))
	{
		ComprDataIO DataIO;
		Unpack Unpack(Arc.m_sParfileName, &DataIO);
		Unpack.Init();

		Array<unsigned char> UnpData(Arc.m_sParfileName, Arc.StreamHead.UnpSize);
	...
	File HostFile(Arc.m_pvRarFiles, Arc.m_sParfileName);
