...
ScanTree::ScanTree(string sParfileName, StringList *FileMasks,int Recurse,bool GetLinks,int GetDirs)
{
...
int ScanTree::FindProc(FindData *FindData)
{
	...
	if (FindStack[Depth]==NULL)
	{
		...
		else
		{
			...
			if (!FindCode)
			{
				ErrHandler.OpenErrorMsg(m_sParfileName, ErrArcName,CurMask);
	...
	if (!FastFindFile && !FindStack[Depth]->Next(FindData,GetLinks))
	{
		...
		if (Error)
		{
			Log(m_sParfileName,NULL,St(MScanError),CurMask);
		}
#endif
	...
	if (FindData->IsDir)
	{
		...
		{
#ifndef SILENT
			Log(m_sParfileName, NULL,"%s%c%s",CurMask,CPATHDIVIDER,Mask);
			Log(m_sParfileName, NULL,St(MPathTooLong));
#endif
			return(SCAN_ERROR);
		}
