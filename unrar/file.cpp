...
File::File(vector<string> *pvRarFiles, string sParfileName)
{
...
bool File::WOpen(const char *Name,const wchar *NameW)
{
	if (Open(Name,NameW))
		return(true);
	ErrHandler.OpenErrorMsg(m_sParfileName, Name);
	return(false);
}
...
bool File::WCreate(const char *Name,const wchar *NameW)
{
	...
	ErrHandler.CreateErrorMsg(m_sParfileName, Name);
	return(false);
}
	...
	if (HandleType!=FILE_HANDLENORMAL)
		...
		if (hFile!=BAD_HANDLE)
		{
			...
			if (!Success && AllowExceptions)
				ErrHandler.CloseError(m_sParfileName, FileName);
		}
		CloseCount++;
		return(Success);
}
	...
	while (1)
	{
		...
		if (!Success && AllowExceptions && HandleType==FILE_HANDLENORMAL)
		{
			...
			if (FreeSize>Size && FilePos-Size<=0xffffffff && FilePos+Size>0xffffffff)
				ErrHandler.WriteErrorFAT(m_sParfileName, FileName);
			if (ErrHandler.AskRepeatWrite(m_pvRarFiles, m_sParfileName, FileName))
			{
				continue;
			}
			ErrHandler.WriteError(m_sParfileName, NULL,FileName);
		}
		break;
	}
	LastWrite=true;
	...
	while (true)
	{
		...
		if (ReadSize==-1)
		{
			...
			if (AllowExceptions)
				...
				{
					if (HandleType==FILE_HANDLENORMAL && ErrHandler.AskRepeatRead(m_pvRarFiles, m_sParfileName, FileName))
						continue;
					ErrHandler.ReadError(m_sParfileName, FileName);
				}
		}
		break;
	}
...
void File::Seek(Int64 Offset,int Method)
{
	if (!RawSeek(Offset,Method) && AllowExceptions)
		ErrHandler.SeekError(m_sParfileName, FileName);
}
...
{
	Array<char> Buffer(m_sParfileName, 0x10000);
	long CopySize=0;
