...
CommandData::CommandData(vector<string> *pvRarFiles, string sParfileName) : RAROptions(sParfileName)
{
...
void CommandData::Init()
{
	...
	*ArcNameW=0;
	...
	FileArgs=new StringList(m_sParfileName);
	ExclArgs=new StringList(m_sParfileName);
	StoreArgs=new StringList(m_sParfileName);
	ArcNames=new StringList(m_sParfileName);
}
			...
			else
			{
				...
				if ((IsDriveDiv(EndChar) || IsPathDiv(EndChar)) && !Add)
					...
					else
					{
						...
						{
							ReadTextFile(m_pvRarFiles, m_sParfileName,Arg+1,FileArgs,false,true,true,true,true);
							FileLists=true;
						}
						else
...
{
	StringList List(m_sParfileName);
	if (ReadTextFile(m_pvRarFiles, m_sParfileName,DefConfigName,&List,true))
	{
		char *Str;
...
void CommandData::ProcessSwitch(char *Switch)
{
	...
	case 'X':
		if (Switch[1]!=0)
			if (Switch[1]=='@' && !IsWildcard(Switch))
				ReadTextFile(m_pvRarFiles, m_sParfileName,Switch+2,ExclArgs,false,true,true,true,true);
			else
	...
	case 'P':
		if (Switch[1]==0)
		{
			GetPassword(m_sParfileName, PASSWORD_GLOBAL,NULL,Password,sizeof(Password));
			eprintf(m_sParfileName,"\n");
		}
		else
...
{
	mprintf(m_sParfileName,St(MUnknownOption),Switch);
...
void CommandData::OutTitle()
{
	...
	mprintf(m_sParfileName,St(MCopyrightS));
#else
#ifndef SILENT
...
#ifdef UNRAR
	mprintf(m_sParfileName,St(MUCopyright),Version,RARVER_YEAR);
#else
#endif
#endif
#endif
...
#endif
#endif
			mprintf(m_sParfileName,St(Help[I]));
	}
...
void CommandData::ProcessCommand()
{
	...
	{
		StringList ArcMasks(m_sParfileName);
		ArcMasks.AddString(ArcName);
		ScanTree Scan(m_sParfileName, &ArcMasks,Recurse,SaveLinks,SCAN_SKIPDIRS);
		FindData FindData;
	...
	case 'I':
		{
			CmdExtract Extract(m_pvRarFiles, m_sParfileName);
	...
	case 'V':
	case 'L':
		ListArchive(m_pvRarFiles, m_sParfileName,this);
		break;
	default:
		OutHelp();
#endif
