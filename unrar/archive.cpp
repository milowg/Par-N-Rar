...
Archive::Archive(vector<string> *pvRarFiles, string sParfileName, RAROptions *InitCmd) : File(pvRarFiles, sParfileName), DummyCmd(sParfileName), NewLhd(sParfileName), SubHead(sParfileName)
{
...
void Archive::CheckArc(bool EnableBroken)
{
	...
	{
		Log(m_sParfileName, FileName,St(MBadArc),FileName);
...
bool Archive::WCheckOpen(char *Name,wchar *NameW)
{
	...
	{
#ifndef SHELL_EXT
		Log(m_sParfileName, FileName,St(MNotRAR),FileName);
#endif
		Close();
		return(false);
	}
...
bool Archive::IsArchive(bool EnableBroken)
{
	...
	{
#ifndef SHELL_EXT
		Log(m_sParfileName,FileName,St(MInvalidName),FileName);
#endif
		return(false);
	}
#endif
	...
	{
		Array<char> Buffer(m_sParfileName, 0x40000);
...
#endif
	{
		...
		{
#ifndef SHELL_EXT
			Log(m_sParfileName, FileName,St(MLogMainHead));
#endif
			Alarm();
			if (!EnableBroken)
				return(false);
