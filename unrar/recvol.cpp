...
RecVolumes::RecVolumes(vector<string> *pvRarFiles, string sParfileName) : Buf(sParfileName)
{
...
bool RecVolumes::Restore(RAROptions *Cmd,const char *Name,const wchar *NameW,bool Silent)
{
	...
	{
#ifndef SILENT
		Log(m_sParfileName,ArcName,St(MNotVolume),ArcName);
#endif
		return(false);
	}
	bool NewNumbering=(Arc.NewMhd.Flags & MHD_NEWNUMBERING);
	Arc.Close();
	char *VolNumStart=VolNameToFirstName(Arc.m_pvRarFiles, Arc.m_sParfileName,ArcName,ArcName,NewNumbering);
	char RecVolMask[NM];
	...
	while (Find.Next(&RecData))
	{
		...
		if (NewStyle)
		{
			...
			{
#ifndef SILENT
				mprintf(m_sParfileName,St(MCRCFailed),Name);
#endif
				continue;
			}
		}
		...
		{
#ifndef SILENT
			Log(m_sParfileName,NULL,St(MRecVolDiffSets),Name,PrevName);
#endif
			return(false);
		}
		RecVolNumber=P[1];
		...
		File *NewFile=new File(m_pvRarFiles, m_sParfileName);
		NewFile->TOpen(Name);
	...
	{
		mprintf(m_sParfileName,St(MRecVolFound),FoundRecVolumes);
	}
#endif
	...
	*LastVolName=0;
	...
	{
		Archive *NewFile=new Archive(m_pvRarFiles, m_sParfileName);
		...
		if (ValidVolume)
		{
				...
				while (!EndFound && NewFile->ReadHeader()!=0)
				{
					...
					if (NewFile->GetHeaderType()==ENDARC_HEAD)
					{
							...
							NewFile->EndArcHead.ArcDataCRC!=CalcFileCRC(NewFile,NewFile->CurBlockPos))
						{
							ValidVolume=false;
#ifndef SILENT
							mprintf(m_sParfileName,St(MCRCFailed),ArcName);
#endif
						}
						EndFound=true;
					}
			...
			if (!ValidVolume)
			{
				...
				strcat(NewName,".bad");
#ifndef SILENT
				mprintf(m_sParfileName,St(MBadArc),ArcName);
				mprintf(m_sParfileName,St(MRenaming),ArcName,NewName);
#endif
...
#ifndef SILENT
			mprintf(m_sParfileName,St(MAbsNextVol),ArcName);
#endif
		}
...
#ifndef SILENT
	mprintf(m_sParfileName,St(MRecVolMissing),MissingVolumes);
#endif

	if (MissingVolumes==0)
	{
#ifndef SILENT
		mprintf(m_sParfileName,St(MRecVolAllExist));
#endif
		return(false);
	}
	...
	{
#ifndef SILENT
		mprintf(m_sParfileName,St(MRecVolCannotFix));
#endif
		return(false);
	}
#ifndef SILENT
	mprintf(m_sParfileName,St(MReconstructing));
#endif
	...
	while (true)
	{
		...
		for (int I=0;I<TotalFiles;I++)
			...
			{
				mprintf(m_sParfileName,"\b\b\b\b%3d%%",CurPercent);
	...
	for (int I=0;I<RecVolNumber+FileNumber;I++)
		...
		mprintf(m_sParfileName,"\b\b\b\b100%%");
		if (!Silent)
