...
int Archive::ReadHeader()
{
	...
	RawRead Raw(m_sParfileName, this);
...
#else
			if (!GetPassword(m_sParfileName, PASSWORD_ARCHIVE,FileName,Cmd->Password,sizeof(Cmd->Password)))
			{
				Close();
				ErrHandler.Exit(USER_BREAK);
			}
	...
	if (Raw.Size()==0)
	{
		...
		{
#ifndef SHELL_EXT
			Log(m_sParfileName, FileName,St(MLogUnexpEOF));
#endif
	...
	{
#ifndef SHELL_EXT
		Log(m_sParfileName, FileName,St(MLogFileHead),"???");
#endif
			...
			*(BaseBlock *)hd=ShortBlock;
			...
			if (hd->HeadType==NEWSUB_HEAD)
			{
				...
				if (hd->HeadCRC!=HeaderCRC)
				{
					...
					ErrHandler.SetErrorCode(WARNING);
#ifndef SHELL_EXT
					Log(m_sParfileName,Archive::FileName,St(MLogFileHead),IntNameToExt(hd->FileName));
					Alarm();
#endif
				}
		}
	...
	if (Decrypt)
	{
		...
		if (ShortBlock.HeadCRC!=HeaderCRC)
		{
			...
			{
#ifndef SILENT
				Log(m_sParfileName,FileName,St(MEncrBadCRC),FileName);
#endif
				Close();
...
bool Archive::ReadSubData(Array<byte> *UnpData,File *DestFile)
{
	...
	{
#ifndef SHELL_EXT
		Log(m_sParfileName, FileName,St(MSubHeadCorrupt));
#endif
		ErrHandler.SetErrorCode(CRC_ERROR);
		return(false);
	}
	...
	{
#ifndef SHELL_EXT
		Log(m_sParfileName, FileName,St(MSubHeadUnknown));
#endif
		return(false);
	}
	...
	SubDataIO.Init();
	Unpack Unpack(m_sParfileName, &SubDataIO);
	Unpack.Init();
	...
	{
#ifndef SHELL_EXT
		Log(m_sParfileName,FileName,St(MSubHeadDataCRC),SubHead.FileName);
#endif
		ErrHandler.SetErrorCode(CRC_ERROR);
		if (UnpData!=NULL)
			UnpData->Reset();
